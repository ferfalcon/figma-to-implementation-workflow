...
## Maintained Astro starter

Application source lives in `starters/astro/`; its dependencies remain separate from the dependency-free CLI. Generate a project with:

```bash
node scripts/build-consumer-bundle.mjs --starter astro --revision <40-character-toolkit-commit-sha>
```

The builder emits the normal lockfile and ignore file from `package-lock.template.json` and `gitignore.template`. These canonical names preserve them through npm packaging, which excludes lockfiles and ignore files. Update dependencies in a generated project with npm, then copy the resulting package manifest and lockfile back to canonical sources, keeping the lockfile's template filename. Never commit installed dependencies or generated application output.

`npm run validate` covers packaging, preferences, remote completion, product evidence, and starter regression tests on Node 22 and 24. The Validate Astro starter workflow independently installs, type-checks, builds, and runs the generated application in desktop and mobile browsers. Extend these checks as maintained interactions change.

The generated Astro starter is a toolkit-controlled implementation fixture. It is not published as a companion template or public release asset. Follow [Product Acceptance](workflow/Product-Acceptance.md) for two real ordinary-ChatGPT sessions. CI passing does not substitute for those sessions or actual Vercel preview review.
