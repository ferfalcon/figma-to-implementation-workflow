...
`npm run validate` covers packaging, preferences, remote completion, evidence, and publisher regression tests on Node 22 and 24. The Validate Astro starter workflow independently installs, type-checks, builds, and runs the generated application in desktop and mobile browsers. Extend these checks as maintained interactions change.

The companion GitHub template is generated by the guarded release workflow. Do not edit it directly. Follow [Product Acceptance and Publication](workflow/Product-Acceptance.md) for two real ordinary-ChatGPT sessions and release evidence. CI passing does not substitute for those sessions or actual Vercel preview review.
