...
try {
  ...
  const record = {
    ...
    snapshots: [{
      ...
      reference: '.', commit: baselineCommit,
    }],
    artifacts: [{ id: 'ART-PLAN', type: 'PLAN', path: 'PLAN.md', status: 'Approved', baseline: ['SRC-REPO-001'] }],
  ...
  const firstStart = resolveTaskStartBaseline(recordPath, record, task1, { cwd });
  assert(firstStart.createdSnapshot, 'First task did not capture the committed planning checkpoint.');
  const firstStartSnapshot = record.snapshots.find((item) => item.id === task1.baseline);
  assert(firstStartSnapshot?.role === 'Task start', 'First task baseline is not a Task start snapshot.');
  assert(firstStartSnapshot?.commit === planCommit, 'First task start did not pin actual HEAD.');
  ...
  record.snapshots.push({
    id: 'SRC-REPO-003', role: 'Implementation output', pinStrength: 'Immutable', status: 'Active',
    reference: '.', commit: outputCommit, parent: task1.baseline, task: task1.id,
  });
  ...
  const secondStart = resolveTaskStartBaseline(recordPath, record, task2, { cwd });
  assert(secondStart.createdSnapshot, 'Second task did not capture the workflow-control checkpoint.');
  const secondStartSnapshot = record.snapshots.find((item) => item.id === task2.baseline);
  assert(secondStartSnapshot?.commit === controlCommit, 'Second task start did not pin actual HEAD.');
  ...
  expectThrow(
    () => resolveTaskStartBaseline(recordPath, record, task3, { cwd }),
  ...
  console.log('Sequential task-start lineage and checkpoint tests passed.');
} finally {
