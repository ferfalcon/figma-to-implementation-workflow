...
7. Use matching guidelines and templates; do not copy generic teaching content into project artifacts.
8. Classify important information as Confirmed, Observed, Inferred, Recommended, or Open question.
9. Document source conflicts instead of silently choosing an interpretation.
...
14. Never report validation as passed unless it was executed successfully.
...
Verify active design inputs, then create or update `DESIGN-AUDIT.md`.
...
Create `IMPLEMENTATION-BRIEF.md`. Keep requirements, design, specification, and planning in separate ownership sections using normal namespaces and pinned snapshot metadata.
...
Verify the repository input snapshot before planning. For Standard and Full, create or update `PLAN.md`. For Lite, complete the plan section of `IMPLEMENTATION-BRIEF.md`.
...
Standard and Full require `TASKS-INDEX.md` and task files. Lite may use one task file for a single coherent result.
...
Create or update `IMPLEMENTATION-REVIEW.md` and compare the result against exact input snapshots, approved artifacts, the pinned Implementation output repository snapshot, and the Validation runtime snapshot.
